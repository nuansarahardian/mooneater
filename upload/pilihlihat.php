...
$order = query("SELECT * FROM ((order_list INNER JOIN daftar_paket ON order_list.id_paket = daftar_paket.id_paket) INNER JOIN status ON order_list.id_status = status.id_status) WHERE order_list.id_status = 3");
          ...
          <table class="table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><?= $row["status"];?></td>
                <td>
                <a href="lihatfotocustomer.php?id=<?= $row["id_order"];?>"><button type="button" class="btn btn-primary btn-sm">See Photos</button></a>
              </td>
              </tr>
              <?php $i++;?>
              <?php endforeach;?>
